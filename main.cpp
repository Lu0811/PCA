...
namespace kdt
{
    ...
    public:
        ...
        void initializeRandomCentroids(const vector<Point3D> &points, int k, vector<Point3D> &centroids)
        {
            vector<Point3D> shuffledPoints = points;
        ...
        // Dentro de la clase KDTree, añade el siguiente método público:
        void kMeans(vector<Point3D> &puntos)
        {
            int maxIteraciones = 100;
            int k = 3;
            // cout << "llEGa a ka funcion csvvvvvvvvvvvv\n";
            //  Inicializar centroides de forma aleatoria
            vector<Point3D> centroides;
            initializeRandomCentroids(puntos, k, centroides);

            // Bucle principal de K-means
            for (int iteracion = 0; iteracion < maxIteraciones; ++iteracion)
            {
                cout << "llEGa a ka funcion csvvvvvvvvvvvv\n";
                // Asignar cada punto al centroide más cercano usando KD-Tree KNN
                vector<vector<Point3D>> clusters(k);
                for (const auto &punto : puntos)
                {
                    vector<Point3D> centroideMasCercano = searchKNN(punto, 1);       // Utiliza la función miembro del objeto actual
                    int indiceCluster = static_cast<int>(centroideMasCercano[0][0]); // Asegurar que sea un número entero
                ...
                for (int i = 0; i < k; ++i)
                {
                    Point3D suma(centroides[i].size(), 0.0);

                    // Sumar las coordenadas de todos los puntos en el clúster
                    for (const auto &punto : clusters[i])
                    {
                        transform(suma.begin(), suma.end(), punto.begin(), suma.begin(), plus<double>());
                    }
                    cout << "llEGa a ka funcion csvvvvvvvvvvvv1\n";
                    // Dividir por el tamaño del clúster para obtener el nuevo centroide
                    transform(centroides[i].begin(), centroides[i].end(), suma.begin(), centroides[i].begin(),
                              [size = clusters[i].size()](double coordCentroide, double coordSuma)
                              {
                                  return coordSuma / size;
                                  cout << "llEGa a ka funcion csvvvvvvvvvvvv2\n";
                              });
                }
                cout << "llEGa a ka funcion csvvvvvvvvvvvv\n";
                // Guardar puntos en CSV en cada iteración
                for (int i = 0; i < k; ++i)
                {
                    // saveClusterToCSV(clusters[i], "iteracion" + to_string(iteracion) + "_cluster" + to_string(i) + ".csv");
                    saveClusterToCSV(clusters[i], "ARCHIVOOOOOOOO.csv");
                }
            }
        }

        void saveClusterToCSV(const vector<Point3D> &cluster, const string &filename)
        {
            cout << cluster[0][0] << "\n";
            ...
            outputFile.close();
            // cout << "Se guardaron los puntos del cluster en " << filename << "." << endl;
        }
...
template <int dimension>
void displayKDTree(kdt::KDTree<dimension> &kdtree)
{
...
int main()
{
    // Specify the CSV file containing data
    ...
    // kdtree.print();
    cout << "FINNNNNN\n";
    // Insert data from CSV into KDTree
    // insertDataFromCSV<3>(kdtree, csvFilename);
    kdtree.kMeans(data);
    // Display the structure of the KDTree
